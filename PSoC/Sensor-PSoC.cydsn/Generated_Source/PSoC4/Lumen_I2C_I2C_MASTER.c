/*******************************************************************************
* File Name: Lumen_I2C_I2C_MASTER.c
* Version 3.10
*
* Description:
...
/* Master variables */
volatile uint16 Lumen_I2C_mstrStatus;      /* Master Status byte  */
volatile uint8  Lumen_I2C_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * Lumen_I2C_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  Lumen_I2C_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  Lumen_I2C_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * Lumen_I2C_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  Lumen_I2C_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  Lumen_I2C_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  Lumen_I2C_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!Lumen_I2C_CY_SCBIP_V0 && \
    Lumen_I2C_I2C_MULTI_MASTER_SLAVE_CONST && Lumen_I2C_I2C_WAKE_ENABLE_CONST)
    static void Lumen_I2C_I2CMasterDisableEcAm(void);
#endif /* (!Lumen_I2C_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Lumen_I2C_mstrStatus  - used to store current status of I2C Master.
*  Lumen_I2C_state       - used to store current state of software FSM.
*  Lumen_I2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  Lumen_I2C_mstrWrBufPtr - used to store pointer to master write buffer.
*  Lumen_I2C_mstrWrBufIndex - used to current index within master write
*  buffer.
*  Lumen_I2C_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 Lumen_I2C_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = Lumen_I2C_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(Lumen_I2C_CHECK_I2C_FSM_IDLE)
        {
            Lumen_I2C_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = Lumen_I2C_CHECK_I2C_STATUS(Lumen_I2C_I2C_STATUS_BUS_BUSY) ?
                            Lumen_I2C_I2C_MSTR_BUS_BUSY : Lumen_I2C_I2C_MSTR_NO_ERROR;
        }
        else if(Lumen_I2C_CHECK_I2C_FSM_HALT)
        {
            Lumen_I2C_mstrStatus &= (uint16) ~Lumen_I2C_I2C_MSTAT_XFER_HALT;
                              errStatus  = Lumen_I2C_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(Lumen_I2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!Lumen_I2C_CY_SCBIP_V0 && \
        Lumen_I2C_I2C_MULTI_MASTER_SLAVE_CONST && Lumen_I2C_I2C_WAKE_ENABLE_CONST)
            Lumen_I2C_I2CMasterDisableEcAm();
    #endif /* (!Lumen_I2C_CY_SCBIP_V0) */

        /* Set up write transaction */
        Lumen_I2C_state = Lumen_I2C_I2C_FSM_MSTR_WR_ADDR;
        Lumen_I2C_mstrWrBufIndexTmp = 0u;
        Lumen_I2C_mstrWrBufIndex    = 0u;
        Lumen_I2C_mstrWrBufSize     = cnt;
        Lumen_I2C_mstrWrBufPtr      = (volatile uint8 *) wrData;
        Lumen_I2C_mstrControl       = (uint8) mode;

        slaveAddress = Lumen_I2C_GET_I2C_8BIT_ADDRESS(slaveAddress);

        Lumen_I2C_mstrStatus &= (uint16) ~Lumen_I2C_I2C_MSTAT_WR_CMPLT;
        ...
        if(Lumen_I2C_CHECK_I2C_MODE_RESTART(mode))
        {
            Lumen_I2C_I2C_MASTER_GENERATE_RESTART;
            Lumen_I2C_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            Lumen_I2C_TX_FIFO_WR_REG = slaveAddress;
            Lumen_I2C_I2C_MASTER_GENERATE_START;
        }
    }

    Lumen_I2C_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterReadBuf
********************************************************************************
*
* Summary:
...
*  Lumen_I2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  Lumen_I2C_mstrRdBufPtr - used to store pointer to master write buffer.
*  Lumen_I2C_mstrRdBufIndex - used to current index within master write
*  buffer.
*  Lumen_I2C_mstrRdBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 Lumen_I2C_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(Lumen_I2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        Lumen_I2C_state = Lumen_I2C_I2C_FSM_MSTR_RD_ADDR;
        Lumen_I2C_mstrRdBufIndex = 0u;
        Lumen_I2C_mstrRdBufSize  = cnt;
        Lumen_I2C_mstrRdBufPtr   = (volatile uint8 *) rdData;
        Lumen_I2C_mstrControl    = (uint8) mode;

        slaveAddress = (Lumen_I2C_GET_I2C_8BIT_ADDRESS(slaveAddress) | Lumen_I2C_I2C_READ_FLAG);

        Lumen_I2C_mstrStatus &= (uint16) ~Lumen_I2C_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(Lumen_I2C_mstrRdBufSize < Lumen_I2C_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            Lumen_I2C_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterSendStart
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(Lumen_I2C_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(Lumen_I2C_CHECK_I2C_STATUS(Lumen_I2C_I2C_STATUS_BUS_BUSY))
        {
            errStatus = Lumen_I2C_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            Lumen_I2C_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                Lumen_I2C_state = Lumen_I2C_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= Lumen_I2C_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            Lumen_I2C_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            Lumen_I2C_ClearMasterInterruptSource(Lumen_I2C_INTR_MASTER_ALL);

            Lumen_I2C_I2C_MASTER_GENERATE_START;


            while(!Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_ACK      |
                                                      Lumen_I2C_INTR_MASTER_I2C_NACK     |
                                                      Lumen_I2C_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_ACK))
            {
                errStatus = Lumen_I2C_I2C_MSTR_NO_ERROR;
            }
            else if(Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_NACK))
            {
                errStatus = Lumen_I2C_I2C_MSTR_ERR_LB_NAK;
            }
            else if(Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_ARB_LOST))
            {
                Lumen_I2C_state = Lumen_I2C_I2C_FSM_IDLE;
                             errStatus = Lumen_I2C_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = Lumen_I2C_I2C_RESET_ERROR;
            }
            else /* Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                Lumen_I2C_state = Lumen_I2C_I2C_FSM_IDLE;
                             errStatus = Lumen_I2C_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = Lumen_I2C_I2C_RESET_ERROR;
            }

            Lumen_I2C_ClearMasterInterruptSource(Lumen_I2C_INTR_MASTER_I2C_ACK      |
                                                        Lumen_I2C_INTR_MASTER_I2C_NACK     |
                                                        Lumen_I2C_INTR_MASTER_I2C_ARB_LOST |
                                                        Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterSendRestart
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(Lumen_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = Lumen_I2C_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(Lumen_I2C_CHECK_I2C_MASTER_CMD(Lumen_I2C_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterSendStop
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterSendStop(void)
{
    ...
    if(Lumen_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_STOP     |
                                                  Lumen_I2C_INTR_MASTER_I2C_ARB_LOST |
                                                  Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_STOP))
        {
            errStatus = Lumen_I2C_I2C_MSTR_NO_ERROR;
        }
        else if(Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = Lumen_I2C_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = Lumen_I2C_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterWriteByte
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(Lumen_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        Lumen_I2C_TX_FIFO_WR_REG = theByte;
        ...
        else /* Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            Lumen_I2C_state = Lumen_I2C_I2C_FSM_IDLE;
                         errStatus = Lumen_I2C_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = Lumen_I2C_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterReadByte
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = Lumen_I2C_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(Lumen_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!Lumen_I2C_CHECK_INTR_RX(Lumen_I2C_INTR_RX_NOT_EMPTY)) &&
              (!Lumen_I2C_CHECK_INTR_MASTER(Lumen_I2C_INTR_MASTER_I2C_ARB_LOST |
                                                  Lumen_I2C_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(Lumen_I2C_CHECK_INTR_RX(Lumen_I2C_INTR_RX_NOT_EMPTY))
        {
            theByte = Lumen_I2C_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Lumen_I2C_mstrRdBufIndex - used to current index within master read
*  buffer.
*
*******************************************************************************/
uint32 Lumen_I2C_I2CMasterGetReadBufSize(void)
{
    return(Lumen_I2C_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return(Lumen_I2C_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterClearWriteBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterStatus
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterStatus(void)
{
    ...
    status = (uint32) Lumen_I2C_mstrStatus;

    if (Lumen_I2C_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) Lumen_I2C_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CMasterClearStatus
********************************************************************************
*
* Summary:
...
uint32 Lumen_I2C_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) Lumen_I2C_mstrStatus;
    Lumen_I2C_mstrStatus = Lumen_I2C_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: Lumen_I2C_I2CReStartGeneration
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Lumen_I2C_I2CReStartGeneration(void)
{
#if(Lumen_I2C_CY_SCBIP_V0)
    /* Generates Restart use GPIO and scb IP functionality. Ticket ID#143715,
    ID#145238 and ID#173656 */
    uint32 status = Lumen_I2C_I2C_STATUS_REG;

    while(Lumen_I2C_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    Lumen_I2C_SET_I2C_SCL_DR(Lumen_I2C_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    Lumen_I2C_SET_I2C_SCL_HSIOM_SEL(Lumen_I2C_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    Lumen_I2C_CTRL_REG &= (uint32) ~Lumen_I2C_CTRL_ENABLED;

    if(0u != (status & Lumen_I2C_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        Lumen_I2C_SET_I2C_SCL_DR(Lumen_I2C_I2C_SCL_LOW);
        CyDelayUs(Lumen_I2C_I2C_TLOW_TIME); /* Count tLOW */

        Lumen_I2C_SET_I2C_SCL_DR(Lumen_I2C_I2C_SCL_HIGH);
        while(Lumen_I2C_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(Lumen_I2C_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    Lumen_I2C_I2C_MASTER_CMD_REG = Lumen_I2C_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    Lumen_I2C_CTRL_REG |= (uint32) Lumen_I2C_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    Lumen_I2C_SET_I2C_SCL_HSIOM_SEL(Lumen_I2C_HSIOM_I2C_SEL);
    ...
    cmd  = Lumen_I2C_I2C_MASTER_CMD_M_START;
    cmd |= Lumen_I2C_CHECK_I2C_STATUS(Lumen_I2C_I2C_STATUS_M_READ) ?
                (Lumen_I2C_I2C_MASTER_CMD_M_NACK) : (0u);

    Lumen_I2C_I2C_MASTER_CMD_REG = cmd;
    ...
    Lumen_I2C_I2C_MULTI_MASTER_SLAVE_CONST && Lumen_I2C_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: Lumen_I2C_I2CMasterDisableEcAm
    ********************************************************************************
    *
    * Summary:
    ...
    static void Lumen_I2C_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (Lumen_I2C_CTRL_REG & Lumen_I2C_CTRL_EC_AM_MODE))
        {
            ...
            Lumen_I2C_Stop();
            Lumen_I2C_CTRL_REG &= (uint32) ~Lumen_I2C_CTRL_EC_AM_MODE;
            Lumen_I2C_Enable();
        }
    }
#endif /* (!Lumen_I2C_CY_SCBIP_V0) */
